...
let result = transactions.filter(function(transaction){
  if (transaction.type === 'sale' && transaction.items.length >= 5)
  return transaction
});

let newArray = []
let finalCustomerList = result.forEach(function(transaction){
  var obj = {customer: transaction.customer, numOfSales: transaction.items.length};
  newArray.push(obj);
});
