var transactions = [
  ...
  */
  let saleTransactions = transactions.filter(function(transaction){
    return transaction.type === 'sale';
});
let itemsArray = saleTransactions.map(function(elem){
    return elem.items.length;
});
let arrayLength = itemsArray.sort(function(a,b){
    return a - b;
});
let finalResult = arrayLength[0] 
